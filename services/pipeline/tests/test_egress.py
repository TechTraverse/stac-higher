...
def test_ipv6_loopback_blocked(monkeypatch):
    ...
    with pytest.raises(EgressBlocked):
        resolve_pinned("v6-loopback.example")
...
def test_dns_failure_blocks(monkeypatch):
    ...
    with pytest.raises(EgressBlocked):
        resolve_pinned("does-not-resolve.example")
